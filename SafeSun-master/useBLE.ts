...
function useBLE(): BluetoothLowEnergyApi {
    ...
    const scanForPeripherals = () =>
        bleManager.startDeviceScan(null, null, (error, device) => {
            ...
            if (device && (device.name?.includes("XIAO") || device.name?.includes("SafeSun") || device.name?.includes("xiao") || device.name?.includes("Xiao"))) {
    ...
    const onUVIndexUpdate = (
       ...
       // const UVorStep = (hexString: string) => {
            if(Number(data) > 15) {
                setstepCount(data);
            } else setUVIndex(data);
       // }
